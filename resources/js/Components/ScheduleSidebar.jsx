...
export default function ScheduleSidebar({ schedules = [], onScheduleClick }) {
  ...
  const getStatusColor = (status) => {
    const statusColors = {
      in_progress: 'bg-green-500',    // FIXED: Changed from 'active' to 'in_progress'
  ...
  const getStatusText = (status) => {
    const statusTexts = {
      in_progress: 'Sedang Berjalan', // FIXED: Changed from 'active' to 'in_progress'
      pending: 'Menunggu',
      completed: 'Selesai',
  ...
  return (
    <div className="bg-gradient-to-bl from-blue-700 to-blue-500 dark:from-gray-800 dark:to-gray-700 text-white rounded-lg p-6 h-full overflow-hidden flex flex-col">
      ...
      <style dangerouslySetInnerHTML={{
        __html: `
          ...
          /* Dark mode styles */
          .dark .custom-sidebar-scrollbar::-webkit-scrollbar-track {
            background: rgba(255, 255, 255, 0.05);
          }
          .dark .custom-sidebar-scrollbar::-webkit-scrollbar-thumb {
            background: linear-gradient(180deg, rgba(156, 163, 175, 0.8) 0%, rgba(156, 163, 175, 0.4) 100%);
            border: 1px solid rgba(156, 163, 175, 0.1);
          }
          .dark .custom-sidebar-scrollbar::-webkit-scrollbar-thumb:hover {
            background: linear-gradient(180deg, rgba(156, 163, 175, 1) 0%, rgba(156, 163, 175, 0.7) 100%);
          }
          .dark .custom-sidebar-scrollbar {
      ...
      {/* Schedule List with Custom Scrollbar */}
          ...
          <div className="text-center py-8 opacity-75">
            ...
            {schedules.length > 0 && (
              <div className="mt-4 text-xs">
                <p>Sample schedule dates:</p>
                {schedules.slice(0, 3).map(s => (
                  <p key={s.id}>{s.tanggal} - {s.namaEvent}</p>
                ))}
              </div>
            )}
          </div>
      ...
      {/* Footer Info */}
      <div className="mt-4 pt-4 border-t border-white border-opacity-20 text-xs opacity-75 text-center">
