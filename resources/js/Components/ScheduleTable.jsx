...
const ScheduleTable = ({ schedules = [], onAdd, onEdit, onDelete, onFilter }) => {
  ...
  const handleEdit = (schedule) => {
    if (onEdit) {
      ...
      alert(`Edit jadwal: ${schedule.namaEvent}`);
    }
  };

  const handleDelete = (schedule) => {
    const eventName = schedule.namaEvent || 'jadwal ini';
    if (window.confirm(`Hapus ${eventName}?`)) {
      if (onDelete) {
        onDelete(schedule.id);
      } else {
        alert(`${eventName} dihapus!`);
      }
    }
  };
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      'in_progress': { class: 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300', text: 'Berlangsung' },
      'pending': { class: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300', text: 'Menunggu' },
      'completed': { class: 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300', text: 'Selesai' },
      'cancelled': { class: 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300', text: 'Dibatalkan' },
      ...
      <div className="overflow-x-auto bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        <table className="w-full min-w-full">
          <thead className="bg-gray-50 dark:bg-gray-800">
            <tr>
              <th className="text-left py-4 px-4 font-semibold text-gray-700 dark:text-gray-300 w-16">No</th>
              <th
              ...
              <th className="text-center py-4 px-4 font-semibold text-gray-700 dark:text-gray-300 w-32">Aksi</th>
            </tr>
          </thead>
          <tbody className="bg-white dark:bg-gray-900">
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((schedule, index) => (
                ...
                >
                  <td className="py-4 px-4 text-gray-600 dark:text-gray-400">{startIndex + index + 1}</td>
                  ...
                  <td className="py-4 px-4 text-gray-800 dark:text-gray-200 font-medium">
                    {schedule.jamMulai && schedule.jamSelesai
                      ...
                      : formatTime(schedule.jamMulai) || formatTime(schedule.waktu) || '-'
                    }
                  </td>
                  ...
                  <td className="py-4 px-4 text-gray-800 dark:text-gray-200">
                    {schedule.fotografer?.nama || schedule.fotografer || '-'}
                  </td>
                  <td className="py-4 px-4 text-gray-800 dark:text-gray-200">
                    {schedule.editor?.nama || schedule.editor || '-'}
                  </td>
                  <td className="py-4 px-4 text-gray-800 dark:text-gray-200">
                    {schedule.lapangan || '-'}
                  </td>
        ...
        <div className="mt-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between mt-4 text-sm text-gray-600 dark:text-gray-400 gap-2">
            <div>
              ...
              {data.length !== totalItems && ` (difilter dari ${data.length} total)`}
            </div>
